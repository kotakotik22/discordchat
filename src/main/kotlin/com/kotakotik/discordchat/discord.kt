...
fun logInDiscord() {
    CoroutineScope(discordContext).launch {
        ...
        kord.on<ReadyEvent> {
            ...
            launch {
                setUpQueueReceiver(kord, webhook, channel)
            }
            ...
            kord.on<MessageCreateEvent> {
                when (message.channelId.toString()) {
                    Config.Channels.chatChannel -> createMcMessage(webhook)
                    Config.Channels.whitelistChannel -> onMessageInWhitelistChannel()
                }
            }
